...
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.open = df_data["open_price"]
        self.high = df_data["highest_price"]
        self.low = df_data["lowest_price"]
        self.close = df_data["close_price"]
        self.volume = df_data["turnover_vol"]
        self.returns = df_data["chg_pct"]
    ...
    def alpha018(self):
        ...
        return -1 * (
            rank(
                (
                    stddev(abs((self.close - self.open)), 5)
    ...
    def alpha021(self):
        cond_1 = sma(self.close, 8) + stddev(self.close, 8) < sma(
            self.close, 2
        )
    ...
    def alpha024(self):
        cond = (
            delta(sma(self.close, 100), 100) / delay(self.close, 100) <= 0.05
        )
    ...
    def alpha025(self):
        ...
        return rank(
            (
                (((-1 * self.returns) * adv20) * self.vwap)
    ...
    def alpha027(self):
        alpha = rank(
            (sma(correlation(rank(self.volume), rank(self.vwap), 6), 2) / 2.0)
        )
        alpha[alpha > 0.5] = -1
        alpha[alpha <= 0.5] = 1
        return alpha
    ...
    def alpha036(self):
        ...
        return (
            ...
            * rank(
                (
                    ((sma(self.close, 200) / 200) - self.open)
                    * (self.close - self.open)
                )
            )
        )
        ...
        return (
            ...
            * rank(
                ...
                * (
                    1
                    - rank(
                        decay_linear((self.volume / adv20).to_frame(), 9).CLOSE
                    )
                )
            )
    ...
    def alpha057(self):
        return 0 - (
            ...
            * (
                ...
                / decay_linear(
                    rank(ts_argmax(self.close, 30)).to_frame(), 2
                ).CLOSE
            )
        )
    ...
    def alpha064(self):
        ...
        return (
            rank(
                correlation(
                    sma(
                        ((self.open * 0.178404) + (self.low * (1 - 0.178404))),
                        13,
                    ),
                    sma(adv120, 13),
                    17,
    ...
    def alpha071(self):
        ...
        p1 = ts_rank(
            decay_linear(
                correlation(
                    ts_rank(self.close, 3), ts_rank(adv180, 12), 18
                ).to_frame(),
                4,
            ).CLOSE,
            16,
        ...
        p2 = ts_rank(
            decay_linear(
                (
                    rank(
                        ((self.low + self.open) - (self.vwap + self.vwap))
                    ).pow(2)
                ).to_frame(),
                16,
            ).CLOSE,
    ...
    def alpha078(self):
        ...
        return rank(
            correlation(
                ts_sum(
                    ((self.low * 0.352233) + (self.vwap * (1 - 0.352233))), 20
                ),
                ts_sum(adv40, 20),
                7,
            )
    ...
    def alpha083(self):
        return (
            rank(
                delay(
                    ((self.high - self.low) / (ts_sum(self.close, 5) / 5)), 2
                )
            )
    ...
    def alpha088(self):
        ...
        p2 = ts_rank(
            decay_linear(
                correlation(
                    ts_rank(self.close, 8), ts_rank(adv60, 21), 8
                ).to_frame(),
                7,
            ).CLOSE,
            3,
    ...
    def alpha094(self):
        ...
        return (
            rank((self.vwap - ts_min(self.vwap, 12))).pow(
                ts_rank(
                    correlation(ts_rank(self.vwap, 20), ts_rank(adv60, 4), 18),
                    3,
                )
            )
            * -1
    ...
    def alpha096(self):
        ...
        p2 = ts_rank(
            decay_linear(
                ts_argmax(
                    correlation(ts_rank(self.close, 7), ts_rank(adv60, 4), 4),
                    13,
                ).to_frame(),
                14,
            ).CLOSE,
    ...
    def alpha098(self):
        ...
        return rank(
            decay_linear(
                correlation(self.vwap, sma(adv5, 26), 5).to_frame(), 7
            ).CLOSE
        ) - rank(
            decay_linear(
                ts_rank(
                    ts_argmin(
                        correlation(rank(self.open), rank(adv15), 21), 9
                    ),
                    7,
                ).to_frame(),
                8,
