...
if os.path.isfile(df_cache_file):
    ...
    logging.info(" Load df from file, take time: " +
    ...
    if df_load.shape[0] == 0:
        ...
        df_load = df_load.sort_values(
            by=['ticker', 'trade_date']).drop_duplicates()
...
for _start_date in _backtest_sec:
    ...
    df_load = df_load[df_load['trade_date'] > pd.to_datetime(
        _start_date) - timedelta(days=momentum_days+11)]
    ...
    logging.info("设置的_start_date:" + _start_date +
                 " 实际的_start_date:" + str(df['trade_date'].iloc[0]))
    ...
    # 买入收益调整
    df.loc[(pd.notnull(df['trade_time'])) & (df['pos'] == 'big'), 'strategy_chg_pct_adj'] = \
        df['close_price_x']/(df['open_price_x'] * (1 + _trade_rate)) - 1
    df.loc[(pd.notnull(df['trade_time'])) & (df['pos'] == 'small'), 'strategy_chg_pct_adj'] = \
        df['close_price_y']/(df['open_price_y'] * (1 + _trade_rate)) - 1

    # 卖出收益调整
    df.loc[(pd.notnull(df['trade_time'].shift(-1))), 'strategy_chg_pct_adj'] = \
