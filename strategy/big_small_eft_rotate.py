...
postgres_datebase = configs["postgres_database"].data
...
class BigSmallEtfRotateStrategy:
    def __init__(self, start_date:datetime ):
        ...
        #配置文件中加载配置文件
        try:
            if bool(configs["bser_suspend"].data):
                self.is_suspend=True
            else:
                self.is_suspend = False
        except Exception as e:
           logger.warn("bser_suspend propety not found!") 
           self.is_suspend = False

    def set_suspend_status(self, is_suspend:bool):
    ...
    def run_as_daemon(self, start_date:datetime):
        if self.is_suspend:
            logger.info(f"{__name__} has been suspend ")
            # Set db as previous roi
            pass
    ...
    def run_with_plot(self, start_date:datetime):
        ...
        if os.path.isfile(df_cache_file):
            ...
            logger.info(
                " Load df from file, take time: " + str(time.time() - start_time)
            )
        else:
            try:
                sql_command1 = (
                    "select trade_date, ticker,open_price,close_price, chg_pct from stock.fund_day where "
                    + f"(ticker = {_ticker_x} or ticker = {_ticker_y}) "
                    + "and trade_date > to_date( '20120528' , 'yyyymmdd') "
                )
                logger.info(sql_command1)

                df_load = pd.read_sql(sql_command1, conn)

            except:
                logger.error(" load data from postgres failure !")
                exit()

            if df_load.shape[0] == 0:
                logger.warn(" there is no data in !")
            else:
                ts = time.time()
                logger.info(" Load data from DB, takes time: " + str(ts - start_time))
                # 默认排序
                df_load = df_load.sort_values(
                    by=["ticker", "trade_date"]
                ).drop_duplicates()
                df_load.to_pickle(df_cache_file)
                logger.info(" Cache df to file, takes time:-" + str(time.time() - ts))
        for _start_date in _backtest_sec:
            # 因为计算N日动量，前N日是没有数据的，因此要提前N+8日
            #  timedelta +8 意思是隔了4个周末共8天 不考虑法定节假日
            df_load = df_load[
                df_load["trade_date"]
                > pd.to_datetime(_start_date) - timedelta(days=momentum_days + 11)
            ]
            ...
            # 买入收益调整
            df.loc[
                (pd.notnull(df["trade_time"])) & (df["pos"] == "big"),
                "strategy_chg_pct_adj",
            ] = (df["close_price_x"] / (df["open_price_x"] * (1 + _trade_rate)) - 1)
            df.loc[
                (pd.notnull(df["trade_time"])) & (df["pos"] == "small"),
                "strategy_chg_pct_adj",
            ] = (df["close_price_y"] / (df["open_price_y"] * (1 + _trade_rate)) - 1)

            # 卖出收益调整
            df.loc[
                (pd.notnull(df["trade_time"].shift(-1))), "strategy_chg_pct_adj"
            ] = (1 + df["strategy_chg_pct"]) * (1 - _trade_rate) - 1
